...
with mp_pose.Pose(
    ...
    for idx, file in enumerate(IMAGE_FILES):
        ...
        if not results.pose_landmarks:
            if (file.find('first') != -1):
                isErrorReady = 1
            elif (file.find('second') != -1):
                isErrorSwing = 1
            elif (file.find('third') != -1):
                isErrorFinish = 1
            continue
        ...
        if (file.find('second') != -1):
            ...
            eval_swing(abs(round(ones, 2) - PUTTER_SWING[0]), 0, "오른쪽 어깨")
            eval_swing(abs(round(twos, 2) - PUTTER_SWING[1]), 1, "왼쪽 어깨")
            eval_swing(abs(round(threes, 2) - PUTTER_SWING[2]), 2, "오른쪽 발")
            eval_swing(abs(round(fours, 2) - PUTTER_SWING[3]), 3, "왼쪽 발")
    ...
    dict = {4: 'Excellent', 3: 'Excellent', 2: 'Perfect', 1: 'Good', 0: 'Fail'}
    # print('---------- 준비 자세 ----------')
